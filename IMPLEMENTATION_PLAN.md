...
The initial emitter model uses controlled per-particle screen-integrated light rather than an unspecified physical inverse-square law. Thus apparent brightness changes from defocus are defined and testable. A distance attenuation mode could be added later as a separate choice. Shell-edge fading is a deliberate visibility effect and is measured separately from blur conservation.
