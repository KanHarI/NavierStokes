...
async function load(page: Page, manual = false) {
  ...
  await page.evaluate(() => {
    ...
    // The public option is hidden. Exercise the retained integration only in
    // this test fixture so it remains covered for possible reintroduction.
    document.querySelector<HTMLButtonElement>('#touch-gyro')!.hidden = false;
  });
}
